...
> 「共用」= 两个 runtime 都声明、且**必须逐字相同**：存储键是插件级的 `plugin.mimo-tts.<key>`，声明不一致时只有一个生效。`pnpm --dir "$COVEL_REPO" validate:plugin "$PWD/plugins/mimo-tts"` 会挡住漂移。
...
```bash
pnpm exec vitest run plugins/mimo-tts/tests   # vitest run，覆盖 wire + 两个 handler
```
