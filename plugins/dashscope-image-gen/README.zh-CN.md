...
`baseUrl` 优先使用控制台为 workspace 提供的专属地址，将 `<WorkspaceId>` 替换为自己的 workspace ID；旧的 `https://dashscope.aliyuncs.com` 仍可用。框架会容忍 Base URL 已带 `/api/v1` 的形式，不会与 wire 端点重复拼接。参见 [DashScope Base URL](https://help.aliyun.com/en/model-studio/base-url)。
